...
| Native Tauri CDP + Playwright E2E | `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/e2e/run-pulse.ps1 -Mode Tauri -Fixture no_data -RunPlaywright` | Starts the owned frontend/bridge on `1420/1421` before the repo-owned debug binary, isolates WebView2 user data, runs `chromium.connectOverCDP`, validates the no-data Home contract, and cleans only exact owned processes |
