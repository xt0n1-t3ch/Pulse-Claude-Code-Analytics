...
for (const theme of themes) {
  for (const viewport of matrix) {
    ...
    await context.addInitScript(({ selectedTheme, accessSnapshot }) => {
      localStorage.setItem("pulse-theme", selectedTheme);
      const session = {
        session_id: "visual-codex", session_name: "Pulse v1.7 overhaul", project: "cc-discord-presence",
        model: "GPT-5.6 Sol · Extra High · ⚡ Fast", model_id: "gpt-5.6-sol", provider: "codex",
        context_window: "1M", cost: 195.79, cost_available: true, cost_basis: "exact",
        tokens: 267600000, input_tokens: 8500000,
        output_tokens: 809200, cache_write_tokens: 0, cache_read_tokens: 258400000,
        context_used_tokens: 267600, context_window_tokens: 1000000,
      ...
      const usage = {
        ...
        usage_source: {
          ...
          stream_id: "codex-subscription:account",
      ...
      const snapshot = {
        ...
        discord_preview: discordPreview,
        discord_settings: discordSettings,
        plan,
        access: accessSnapshot,
      };
      const history = [{ id: "visual-codex", session_name: "Pulse v1.7 overhaul", project: session.project, provider: "codex", model: session.model, model_id: session.model_id, context_window: "1M", branch: session.branch, effort: session.effort, started_at: session.started_at, ended_at: null, duration_secs: session.duration_secs, total_cost: session.cost, known_cost: session.cost, cost_basis: "exact", cost_source: "provider_billed", input_tokens: session.input_tokens, output_tokens: session.output_tokens, cache_write_tokens: 0, cache_read_tokens: session.cache_read_tokens, total_tokens: session.tokens, input_cost: session.input_cost, output_cost: session.output_cost, cache_write_cost: 0, cache_read_cost: session.cache_read_cost, has_thinking: true, workflow_label: "ULTRACODE", subagent_count: 2, is_active: true }];
      const contextBreakdown = {
        model: session.model,
        context_window: session.context_window_tokens,
        used_tokens: session.context_used_tokens,
        free_space: session.context_window_tokens - session.context_used_tokens,
        autocompact_buffer: 0,
        system_prompt: 8200,
        system_tools: 4800,
        memory_files: [{ name: "AGENTS.md", tokens: 7600 }],
        memory_total: 7600,
        skills: [{ name: "voice", tokens: 5800 }, { name: "research", tokens: 4700 }],
        skills_total: 10500,
        messages: 236500,
        mcp_tools: [{ name: "node_repl", tokens: 9500 }],
        mcp_total: 9500,
      };
      const contextRows = [{
      ...
      const reports = {
        provider: "codex", capabilities: { cache_health: true, model_routing: false, extra_usage: false }, days: 30, total_sessions: 1, priced_sessions: 1, cost_basis: "exact", cost_sources: ["provider_billed"], recommendations: [], inflection_points: [], model_routing: null,
        daily_costs: [{ date: "2026-07-16", cost: 195.79, sessions: 1, priced_sessions: 1, cost_basis: "exact", cost_sources: ["provider_billed"] }],
      ...
      const values = {
        ...
        get_trace_overview: reports.trace_overview,
        get_context_breakdowns: contextRows,
        get_context_breakdown: contextBreakdown,
    ...
    }, { selectedTheme: theme, accessSnapshot: accessFixture.expected_dto });
    ...
    await page.getByRole("button", { name: /Codex Pro 20x Subscription/ }).click();
    ...
    for (const view of views) {
      ...
      if (scopeCommand) {
        const observedScope = await page.evaluate((command) =>
          window.__PULSE_INVOKE_ARGS__?.[command]?.provider ?? null,
          scopeCommand,
        );
        if (observedScope !== "codex") {
          throw new Error(`${view} lost selected provider scope: ${JSON.stringify({ scopeCommand, observedScope })}`);
        }
      }
      const selectedSource = await page.getByRole("button", { name: /Codex Pro 20x Subscription/ }).getAttribute("aria-pressed");
