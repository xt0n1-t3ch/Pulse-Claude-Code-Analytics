...
test("frameless titlebar grants native drag while controls stay stationary", async () => {
  const endpoint = process.env.PULSE_TAURI_CDP_URL;
  if (!endpoint) throw new Error("PULSE_TAURI_CDP_URL is required");
  ...
  const brand = page.locator("header.app-header .brand");
  await expect(brand).toBeVisible();
  const nativeCapability = await page.evaluate(async () => {
  ...
  expect(nativeCapability).not.toMatch(/^rejected:/);
