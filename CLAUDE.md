...
- **GA models** (Opus 4.6+, Sonnet 4.6+): Standard pricing at any context.
  `is_ga_1m_context()` returns `true`.
- **Beta models** (Opus 4.5, Sonnet 4/4.5): 2× input, 1.5× output, 2× cache
  when total API input > 200K.
- Model IDs with `[1m]` suffix are stripped before parsing.
